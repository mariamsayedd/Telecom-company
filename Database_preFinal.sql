...
as
begin 
return(Select  amount from Cashback as cb 
inner join 
Plan_Provides_Benefits as ppb 
on cb.benefit_id = ppb.benefit_id where 
ppb.planID = @planID  and 
cb.walletID = @WalletID)
END
GO
...
from Transfer_money 
where 
transfer_date >= @start_date and transfer_date < @end_date and
walletID2 = @Wallet_id
return @answer
END
GO
...
where p.mobileNo = @MobileNo and p.status = 'Accepted'
